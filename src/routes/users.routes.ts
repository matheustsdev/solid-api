...
usersRoutes.patch('/:user_id/admin', (request, response) => turnUserAdminController.handle(request, response));

usersRoutes.get('/:user_id', (request, response) => showUserProfileController.handle(request, response));
